...
export const createTemplate = secureAction({ schema: createTemplateInput, permission: "templates:manage" }, async (input) => {
...
export const duplicateTemplate = secureAction({ schema: templateIdInput, permission: "templates:manage" }, async ({ id }) => {
  const t = await backend<Template>(`/templates/${id}/duplicate`, { method: "POST" });
  revalidatePath("/templates");
  return { id: t.id };
});

export const deleteTemplate = secureAction({ schema: templateIdInput, permission: "templates:manage" }, async ({ id }) => {
