...
const statusLabel: Record<DnsStatus, string> = {
  pending: "Waiting for DNS",
  verified: "Verified",
  failed: "Not found",
};
const recordIcon: Record<DnsStatus, React.ReactNode> = {
  verified: <CheckCircle2 className="size-4 text-success" role="img" aria-label="Found" />,
  pending: <Clock className="size-4 text-warning" role="img" aria-label="Waiting" />,
  failed: <XCircle className="size-4 text-danger" role="img" aria-label="Not found" />,
};
...
function DomainCard({ domain }: { domain: SendingDomain }) {
  ...
  const verify = useAction(verifySendingDomain, {
    success: (r) =>
      r.status === "verified"
        ? "Domain verified. You can send from it now."
        ...
        title={
          <span className="flex items-center gap-2">
            ...
            <StatusBadge
              ...
              label={statusLabel[domain.status]}
            />
          </span>
        }
        description={
          domain.lastCheckedAt ? (
            <span suppressHydrationWarning>Last checked {formatRelative(domain.lastCheckedAt)}</span>
          ) : undefined
        }
      />
      {domain.status !== "verified" && (
        <CardContent className="grid gap-3">
          <p className="text-sm text-muted-foreground">
            Add these records where your domain is managed (for example your web host or registrar), then
            select <span className="font-medium text-foreground">Check now</span>. Your IT person will know
            what to do.
          </p>
      ...
      <CardFooter className="justify-end">
          ...
          onClick={async () => {
            if (
              await modals.confirm({
                ...
                description: "Campaigns can no longer be sent from addresses on this domain.",
        ...
        {domain.status !== "verified" && (
          <Button
            ...
            onClick={() => verify.run({ id: domain.id })}
          >
            Check now
          </Button>
        )}
...
function AddDomain() {
  ...
  const add = useAction(addSendingDomain, {
    success: "Domain added. Now add the DNS records.",
    onSuccess: () => {
      setOpen(false);
      setDomain("");
    },
        ...
        <DialogContent size="sm">
          ...
          >
            <DialogHeader>
              ...
              <DialogDescription>
                The part after the @ in the address you want to send from.
              </DialogDescription>
            </DialogHeader>
            <DialogBody>
    ...
    <div className="grid gap-page">
        ...
        >
          ...
          <CardContent className="grid gap-5">
            <Field label="Organisation name" htmlFor="s-org" error={errors.organisationName} inline>
              <Input
                id="s-org"
                value={v.organisationName}
                onChange={(e) => setV({ ...v, organisationName: e.target.value })}
                aria-invalid={!!errors.organisationName}
                aria-describedby="s-org-msg"
                maxLength={120}
              />
            </Field>
            <Field label="Postal address" htmlFor="s-addr" error={errors.postalAddress} inline>
              <Textarea
                ...
                onChange={(e) => setV({ ...v, postalAddress: e.target.value })}
                aria-invalid={!!errors.postalAddress}
                aria-describedby="s-addr-msg"
                maxLength={300}
              />
            </Field>
            <Field label="Default sender name" htmlFor="s-from" optional inline>
              <Input
                id="s-from"
                value={v.defaultFromName}
                onChange={(e) => setV({ ...v, defaultFromName: e.target.value })}
                maxLength={80}
              />
            </Field>
            <Field
              ...
              hint="Must be on a verified domain below."
              inline
            >
              <Input
                id="s-fromEmail"
                type="email"
                value={v.defaultFromEmail}
                onChange={(e) => setV({ ...v, defaultFromEmail: e.target.value })}
                aria-describedby="s-fromEmail-msg"
              />
            </Field>
            <Field label="Replies go to" htmlFor="s-reply" optional error={errors.defaultReplyTo} inline>
              <Input
                id="s-reply"
                type="email"
                value={v.defaultReplyTo}
                onChange={(e) => setV({ ...v, defaultReplyTo: e.target.value })}
                aria-describedby="s-reply-msg"
              />
            </Field>
          </CardContent>
          <CardFooter className="justify-end">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="grid gap-0.5">
          ...
          <p className="text-sm text-muted-foreground">
            Verifying your domain stops your emails landing in spam.
          </p>
        </div>
        <AddDomain />
      </div>
      {settings.domains.length === 0 ? (
        <Card variant="outline">
          <EmptyState
            ...
            description="Add the domain your church’s email addresses use, for example esocs.org."
          />
        </Card>
      ) : (
