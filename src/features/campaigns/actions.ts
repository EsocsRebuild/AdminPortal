...
export const createCampaign = secureAction(
  { schema: createCampaignInput, permission: "campaigns:manage" },
  async (input) => {
...
export const estimateRecipients = secureAction(
  { schema: estimateInput, permission: "campaigns:manage" },
  async ({ listIds }) =>
    listIds.length
      ? backend<{ count: number }>("/audiences/estimate", { method: "POST", body: { listIds } })
      : { count: 0 },
);

export const sendTestEmail = secureAction(
  { schema: sendTestInput, permission: "campaigns:manage" },
  async ({ id, emails }) => {
...
async function assertReady(id: string) {
  ...
  if (!check.success)
    throw new BackendError(
      "VALIDATION",
      check.error.issues[0]?.message ?? "This campaign isn’t ready yet.",
      422,
    );
  if (c.status !== "draft")
    throw new BackendError("CONFLICT", "This campaign has already been scheduled or sent.", 409);
}
...
export const sendCampaignNow = secureAction(
  ...
  async ({ id, expectedRecipients }, { sudoHeaders }) => {
    await assertReady(id);
    await backend(`/campaigns/${id}/send`, {
      method: "POST",
...
export const unscheduleCampaign = secureAction(
  { schema: campaignIdInput, permission: "campaigns:send" },
  async ({ id }) => {
...
export const duplicateCampaign = secureAction(
  { schema: campaignIdInput, permission: "campaigns:manage" },
  async ({ id }) => {
...
export const deleteCampaign = secureAction(
  { schema: campaignIdInput, permission: "campaigns:manage" },
  async ({ id }) => {
