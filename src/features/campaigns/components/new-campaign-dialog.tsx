...
export function NewCampaignButton({ templates }: { templates: Template[] }) {
  ...
  const create = useAction(createCampaign, { onSuccess: ({ id }) => router.push(`/campaigns/${id}/edit?step=setup`) });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const content: EmailDocument =
      kind === "t" ? structuredClone(templates.find((t) => t.id === id)!.content) : presets.find((p) => p.id === id)!.build();
    const res = await create.run({ name, content });
    if (!res.ok) setError(res.fieldErrors?.name?.[0]);
  }

  const options = [
    ...templates.map((t) => ({ value: `t:${t.id}`, name: t.name, description: "Your template", doc: t.content })),
    ...presets.map((p) => ({ value: `p:${p.id}`, name: p.name, description: p.description, doc: presetDocs[p.id] })),
  ];
        ...
        <DialogContent size="xl">
          <form onSubmit={submit} noValidate className="flex min-h-0 flex-col">
            <DialogHeader>
              ...
              <DialogDescription>Name it, pick a starting design, and we’ll guide you through the rest. Nothing is sent until you say so.</DialogDescription>
            </DialogHeader>
            <DialogBody className="grid gap-5">
              <Field label="Campaign name" htmlFor="nc-name" error={error} hint="Only your team sees this.">
                <Input id="nc-name" autoFocus placeholder="e.g. October newsletter" value={name} onChange={(e) => { setName(e.target.value); setError(undefined); }} aria-invalid={!!error} aria-describedby="nc-name-msg" maxLength={100} />
              </Field>
              <RadioPrimitive.Root value={start} onValueChange={setStart} aria-label="Starting design" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {options.map((o) => (
