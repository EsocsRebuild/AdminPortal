...
export function EmailThumbnail({ document, className, size = "md" }: { document: EmailDocument; className?: string; size?: "sm" | "md" }) {
  return (
    <div aria-hidden className={cn("pointer-events-none relative h-44 overflow-hidden bg-surface-muted select-none", className)}>
      <div className={cn("absolute top-0 w-[640px]", size === "sm" ? "left-0 origin-top-left scale-[0.275]" : "left-1/2 origin-top -translate-x-1/2 scale-[0.42]")}>
