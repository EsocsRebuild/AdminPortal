        ...
        <form onSubmit={(e) => submit(e)} noValidate className="flex min-h-0 flex-col">
          ...
          <DialogBody className="grid gap-4 sm:grid-cols-2">
            <Field label="Email" htmlFor="c-email" error={errors.email} required className="sm:col-span-2">
              <Input id="c-email" type="email" autoFocus prefix={<Mail />} value={values.email} onChange={(e) => setValues({ ...values, email: e.target.value })} aria-invalid={!!errors.email} aria-describedby="c-email-msg" />
            </Field>
            <Field label="First name" htmlFor="c-first" optional>
              <Input id="c-first" value={values.firstName} onChange={(e) => setValues({ ...values, firstName: e.target.value })} maxLength={80} />
            </Field>
            <Field label="Last name" htmlFor="c-last" optional>
              <Input id="c-last" value={values.lastName} onChange={(e) => setValues({ ...values, lastName: e.target.value })} maxLength={80} />
            </Field>
            <div className="grid gap-1 sm:col-span-2">
              ...
              />
              {errors.consent && <p role="alert" className="text-xs text-danger">{errors.consent}</p>}
            </div>
          </DialogBody>
          <DialogFooter>
