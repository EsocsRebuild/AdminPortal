...
function ImportDialogBody({ listId, onOpenChange }: { listId: string; onOpenChange: (o: boolean) => void }) {
  ...
  const [mapping, setMapping] = React.useState<Record<Field, number | null>>({
    email: null,
    firstName: null,
    lastName: null,
  });
  ...
  async function onFile(file: File) {
    ...
    if (rows.length < 2)
      return setError("That file looks empty. It needs a header row and at least one contact.");
    if (rows.length - 1 > MAX_ROWS)
      return setError(`That’s more than ${formatNumber(MAX_ROWS)} contacts. Split it into smaller files.`);
    ...
    if (guessed.email === null) {
      const i = header.findIndex((_, col) =>
        body.slice(0, 20).some((r) => EMAIL_RE.test(r[col]?.trim() ?? "")),
      );
  ...
  async function startImport() {
    ...
    for (let i = 0; i < total; i += CHUNK) {
      const res = await run.run({
        listId,
        contacts: analysis.valid.slice(i, i + CHUNK),
        consent: true,
        updateExisting,
      });
      if (!res.ok) {
        setError(
          `${res.message} ${formatNumber(i)} of ${formatNumber(total)} contacts were imported before this happened.`,
        );
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Upload a CSV file exported from a spreadsheet, like Excel or Google Sheets.
        </DialogDescription>
      </DialogHeader>
        ...
        {stage.name === "upload" && (
          <>
            <FileDrop
              ...
              onFile={onFile}
              onError={setError}
              hint="CSV up to 5 MB. The first row should be column names."
            />
            <p className="text-sm text-muted-foreground">
              Tip: in Excel choose <span className="font-medium text-foreground">File → Save as → CSV</span>.
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value={NONE}>
                        {f.key === "email" ? "Choose a column" : "Don’t import"}
                      </SelectItem>
              ...
              <>
                ...
                <ul className="grid gap-1.5 text-sm">
                  <li className="flex items-center gap-2">
                    ...
                    <span>
                      <span className="tabular font-semibold">{formatNumber(analysis.valid.length)}</span>{" "}
                      ready to import
                    </span>
                  </li>
                  {analysis.invalid > 0 && (
            ...
            <dl className="grid w-full max-w-sm grid-cols-2 gap-3 text-left">
              {[
                ...
                <div key={k} className="rounded-control bg-surface-muted p-3">
                  ...
                  <dd className="tabular text-lg font-semibold">{formatNumber(Number(v))}</dd>
                </div>
              ))}
            </dl>
          </div>
