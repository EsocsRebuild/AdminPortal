...
export const changePasswordInput = z
  .object({ current: z.string().min(1, { error: "Enter your current password." }).max(128), next: newPassword, confirm: z.string() })
...
export const mfaCodeInput = z.object({ code: z.string().trim().regex(/^\d{6}$/, { error: "Enter the 6-digit code." }) });
