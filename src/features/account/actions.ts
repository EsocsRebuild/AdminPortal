...
export const beginMfaSetup = secureAction(
  { schema: z.object({}), sudo: true },
  async (_input, { sudoHeaders }) => {
    const { secret, otpauthUrl } = await backend<{ secret: string; otpauthUrl: string }>("/me/mfa/setup", {
      method: "POST",
      headers: sudoHeaders,
    });
...
export const enableMfa = secureAction({ schema: mfaCodeInput }, async ({ code }) => {
  const { recoveryCodes } = await backend<{ recoveryCodes: string[] }>("/me/mfa/enable", {
    method: "POST",
    body: { code },
  });
  revalidatePath("/settings/security");
...
export const disableMfa = secureAction(
  { schema: z.object({}), sudo: true },
  async (_input, { sudoHeaders }) => {
...
export const regenerateRecoveryCodes = secureAction(
  { schema: z.object({}), sudo: true },
  async (_input, { sudoHeaders }) => {
    const r = await backend<{ recoveryCodes: string[] }>("/me/mfa/recovery-codes", {
      method: "POST",
      headers: sudoHeaders,
    });
    revalidatePath("/settings/security");
...
export const revokeOtherSessions = secureAction(
  { schema: z.object({}), sudo: true },
  async (_i, { sudoHeaders }) => {
    const r = await backend<{ revoked: number }>("/me/sessions/revoke-others", {
      method: "POST",
      headers: sudoHeaders,
    });
    revalidatePath("/settings/security");
