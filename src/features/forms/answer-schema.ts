    ...
    switch (f.type) {
      ...
      case "email":
        shape[f.id] = f.required
          ? asText(z.string().trim().min(1, { error: REQUIRED }).pipe(z.email({ error: "Enter a valid email address." })))
      ...
      case "phone": {
        ...
        shape[f.id] = f.required ? asText(z.string().trim().min(1, { error: REQUIRED }).pipe(phone)) : z.union([z.literal(""), phone]).transform((v) => v || null).optional();
        break;
      }
      case "number": {
        ...
        shape[f.id] = f.required ? blankToUndefined(z.any().refine((v) => v !== undefined, { error: REQUIRED }).pipe(n)) : blankToUndefined(n.optional());
        break;
      }
      case "date": {
        const d = z.iso.date({ error: "Enter a valid date." });
        shape[f.id] = f.required ? asText(z.string().min(1, { error: REQUIRED }).pipe(d)) : z.union([z.literal(""), d]).transform((v) => v || null).optional();
        break;
      }
      case "select":
      case "radio": {
        const choice = z.string().refine((v) => optionIds.includes(v), { error: "Choose one of the options." });
        shape[f.id] = f.required ? asText(z.string().min(1, { error: "Choose an option." }).pipe(choice)) : z.union([z.literal(""), choice]).transform((v) => v || null).optional();
        break;
      }
      case "checkboxes": {
        const arr = z.array(z.string().refine((v) => optionIds.includes(v))).max(optionIds.length);
        shape[f.id] = z.preprocess((v) => v ?? [], f.required ? arr.min(1, { error: "Choose at least one option." }) : arr);
        break;
      }
      case "consent":
        shape[f.id] = z.preprocess((v) => v ?? false, f.required ? z.literal(true, { error: "Please tick this box to continue." }) : z.boolean());
        break;
    }
  }
