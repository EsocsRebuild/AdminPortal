...
export const createForm = secureAction({ schema: createFormInput, permission: "forms:manage" }, async (input) => {
  const f = await backend<Form>("/forms", { method: "POST", body: input });
  revalidatePath("/forms");
  return { id: f.id };
});

export const updateForm = secureAction({ schema: updateFormInput, permission: "forms:manage" }, async ({ id, ...patch }) => {
  await backend(`/forms/${id}`, { method: "PATCH", body: patch });
  return null;
});

export const updateFormSettings = secureAction({ schema: settingsInput, permission: "forms:manage" }, async ({ id, settings }) => {
  await backend(`/forms/${id}/settings`, { method: "PUT", body: settings });
  paths(id);
  return null;
});

export const updateFormSlug = secureAction({ schema: slugInput, permission: "forms:manage" }, async ({ id, slug }) => {
  await backend(`/forms/${id}/slug`, { method: "PUT", body: { slug } });
  paths(id);
  return { slug };
});

export const publishForm = secureAction({ schema: formIdInput, permission: "forms:manage" }, async ({ id }) => {
  const form = await backend<Form>(`/forms/${id}`);
  const check = publishableSchema.safeParse(form);
  if (!check.success) throw new BackendError("VALIDATION", check.error.issues[0]?.message ?? "This form isn’t ready yet.", 422);
...
export const duplicateForm = secureAction({ schema: formIdInput, permission: "forms:manage" }, async ({ id }) => {
...
export const deleteForm = secureAction({ schema: formIdInput, permission: "forms:manage", sudo: true }, async ({ id }, { sudoHeaders }) => {
...
export const deleteResponses = secureAction(
  ...
  async ({ id, responseIds }, { sudoHeaders }) => {
    const r = await backend<{ deleted: number }>(`/forms/${id}/responses/delete`, { method: "POST", body: { responseIds }, headers: sudoHeaders });
    paths(id);
    return r;
  },
);
 ...
 */
export const submitFormResponse = publicAction({ schema: publicSubmitInput }, async ({ slug, answers, website, startedAt, captchaToken }) => {
  if (website) return { ok: true as const }; // Honeypot hit: pretend success, store nothing.
  if (Date.now() - startedAt < 2500) throw new BackendError("RATE_LIMITED", "That was quick! Please check your answers and submit again.", 429);

  const form = await backend<PublicForm>(`/public/forms/${slug}`, { auth: false });
  if (form.status !== "published") throw new BackendError("CONFLICT", "This form is no longer accepting responses.", 409);
  ...
  await backend(`/public/forms/${slug}/responses`, { method: "POST", auth: false, body: { answers: parsed.data, captchaToken } });
