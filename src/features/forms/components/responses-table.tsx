...
export function ResponsesTable({ form, page, server }: { form: Form; page: FormResponse[]; server: ServerTableState }) {
  const modals = useModals();
  const canManage = usePermission("forms:manage");
  const [open, setOpen] = React.useState<FormResponse | null>(null);
  const remove = useAction(deleteResponses, { success: (r) => `${pluralize(r.deleted, "response")} deleted` });
    ...
    const cols: DataTableColumn<FormResponse>[] = [
      ...
      ...questions.slice(0, 6).map((q) =>
        col.display({
          ...
          cell: ({ row }) => <span className="block max-w-64 truncate">{formatAnswer(q, row.original.answers[q.id]) || "—"}</span>,
        }),
      ),
    ];
    return cols;
      ...
      <DataTable
        ...
        renderMobileRow={(row) => {
          ...
          return (
            <button type="button" onClick={() => setOpen(r)} className="grid w-full gap-1 p-3.5 text-left">
              <span className="truncate font-medium">{(first && formatAnswer(first, r.answers[first.id])) || "Response"}</span>
        ...
        emptyState={
          <EmptyState
            ...
            description={form.status === "published" ? "Share the link from the Share tab to start collecting answers." : "Publish the form and share its link to start collecting answers."}
          />
        }
      />
            ...
            <>
              ...
              <SheetBody>
                <dl className="grid gap-5">
                  {form.fields.map((f) =>
                    f.type === "section" ? (
                      <h3 key={f.id} className="border-t border-border pt-4 text-sm font-semibold first:border-0 first:pt-0">
                        {f.label}
                      </h3>
                    ) : (
                      <div key={f.id} className="grid gap-1">
                        <dt className="text-sm text-muted-foreground">{f.label || "Untitled question"}</dt>
                        <dd className="text-base whitespace-pre-wrap">{formatAnswer(f, open.answers[f.id]) || <span className="text-subtle-foreground">No answer</span>}</dd>
                      </div>
                    ),
                  )}
                </dl>
