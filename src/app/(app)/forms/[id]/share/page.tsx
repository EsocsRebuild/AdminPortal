...
export default async function ShareFormPage({ params }: PageProps<"/forms/[id]/share">) {
  ...
  const svg = await QRCode.toString(url, { type: "svg", margin: 1, errorCorrectionLevel: "M", color: { dark: "#0f172a", light: "#ffffff" } });
  ...
  return (
    <div className="grid gap-page">
      {form.status !== "published" && (
        <Alert tone="info" title={form.status === "draft" ? "This form isn’t published yet" : "This form is closed"}>
      ...
      <div className="grid gap-page lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="grid content-start gap-page">
          <Card>
            <CardHeader title="Share a link" description="Send it by email, WhatsApp or text, or post it on social media." />
            <CardContent className="grid gap-3">
              ...
              <Button variant="secondary" size="sm" className="w-fit" asChild>
                <a href={`https://wa.me/?text=${encodeURIComponent(`${form.title} ${url}`)}`} target="_blank" rel="noopener noreferrer">
                  Share on WhatsApp
                </a>
              </Button>
            </CardContent>
          ...
          <Card>
            <CardHeader title="Put it on your website" description="Paste this code where the form should appear." />
            <CardContent className="grid gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                Only websites your administrator has allowed can embed forms. Ask them to add your site if it doesn’t appear.
              </p>
            </CardContent>
          </Card>
        </div>
        <Card className="h-fit">
          ...
          <CardContent className="grid justify-items-center gap-4">
            ...
            <img src={dataUrl} alt={`QR code linking to ${url}`} className="size-52 rounded-control border border-border bg-white p-2" />
