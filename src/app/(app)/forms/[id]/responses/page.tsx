...
export default async function ResponsesPage({ params, searchParams }: PageProps<"/forms/[id]/responses">) {
  ...
  const [form, responses] = await Promise.all([findOrNotFound(getForm(id)), listResponses(id, { ...query, sort: query.sort ?? "submittedAt", dir: query.dir ?? "desc" })]);
  return (
    <ResponsesTable
      form={form}
      page={responses.data}
      server={{ total: responses.meta.total, page: responses.meta.page, pageSize: responses.meta.pageSize, q: query.q, sort: query.sort, dir: query.dir }}
    />
  );
}
