...
export function SectionNav({ items, label }: { items: SectionNavItem[]; label: string }) {
  ...
  return (
    <nav aria-label={label} className="lg:sticky lg:top-[calc(var(--spacing-topbar)+1.5rem)] lg:self-start">
      <ul className="scrollbar-none mask-fade-x flex gap-1 overflow-x-auto px-1 lg:grid lg:px-0 lg:[mask-image:none]">
        {items
              ...
              <li key={i.href} className="shrink-0">
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-surface font-medium text-foreground shadow-xs ring-1 ring-border" : "text-muted-foreground hover:bg-surface-hover hover:text-foreground",
                  )}
                >
                  {i.label}
                </Link>
